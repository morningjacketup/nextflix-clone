				...
				{data.map((movie) => (
					...
					>
						...
						<div className='relative h-60 z-10 w-full transform transition duration-500 hover:scale-125 opacity-0 hover:opacity-100'>
							<div className='bg-gradient-to-b from-transparent via-black/50 z-10 w-full h-full rounded-lg flex items-center justify-center'>
								<Image
									src={movie.Movie?.imageString as string}
									alt={movie.Movie?.title as string}
									width={800}
								...
								<MovieCard
									...
									watchListId={
										movie.Movie?.WatchLists[0]?.id as string
									}
									watchList={
										(movie.Movie?.WatchLists
											.length as number) > 0
											? true
											: false
									}
									...
									youtubeUrl={
										movie.Movie?.youtubeString as string
									}
								/>
							</div>
						</div>
